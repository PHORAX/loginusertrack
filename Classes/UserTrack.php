	...
	function moduleContent()	{
		...
		switch((string)$this->MOD_SETTINGS['function'])	{
			case 1:  // Login log
				...
				while(($row=$GLOBALS['TYPO3_DB']->sql_fetch_assoc($res))) {
					...
					$list[]='<tr bgcolor="'.$this->doc->bgColor4.'">
						...
						<td nowrap>'.BackendUtility::calcAge(time()-$row['session_login'],$GLOBALS['LANG']->sL('LLL:EXT:lang/locallang_core.php:labels.minutesHoursDaysYears')).'</td>
						<td nowrap><a href="' . BackendUtility::getModuleUrl($this->MCONF['name'], $modParam) . '">'.$row['username'].'</a></td>
						<td nowrap>'.$row['name'].'</td>
						<td nowrap>'.BackendUtility::calcAge($row['last_page_hit']-$row['session_login'],$GLOBALS['LANG']->sL('LLL:EXT:lang/locallang_core.php:labels.minutesHoursDaysYears')).'</td>
			...
			case 3:  // Monthly view
				...
				for ($a=0;$a<12;$a++)	{
					...
					while (($row=$GLOBALS['TYPO3_DB']->sql_fetch_assoc($res))) {
						$list[]='<tr bgcolor="'.$this->doc->bgColor4.'">
							...
							<td nowrap>'.BackendUtility::calcAge(time()-$row['last_login'],$GLOBALS['LANG']->sL('LLL:EXT:lang/locallang_core.php:labels.minutesHoursDaysYears')).'</td>
							<td nowrap><a href="' . BackendUtility::getModuleUrl($this->MCONF['name'], array('id' => $this->id, 'useruid' => $row['user_uid'])) . '">'.$row['username'].'</a></td>
							<td nowrap>'.$row['name'].'</td>
							<td>'.$row['counter'].'</td>
						</tr>
						';
					...
					$content.='
					<BR>
					'.$GLOBALS['LANG']->getLL('period').' <strong>'.date($GLOBALS['TYPO3_CONF_VARS']['SYS']['ddmmyy'],$times[$a+1]).' - '.date($GLOBALS['TYPO3_CONF_VARS']['SYS']['ddmmyy'],$times[$a]-1).'</strong><BR>
				...
				if ($userId>0)	{
					$content='<a href="' . BackendUtility::getModuleUrl($this->MCONF['name'], array('id' => $this->id)) . '"><strong>'.$GLOBALS['LANG']->getLL('modulecont_listAllUsers').'</strong></a><br>'.$content;
				}
