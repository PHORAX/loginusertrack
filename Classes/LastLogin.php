...
class LastLogin {
	...
	function main($id,&$pObj,$mode)	{
		...
		$content.='<strong>'.$GLOBALS['LANG']->getLL('lastlogin_main_totalNumberOfUsers','1').':</strong> '.$row['t'];
		...
		if ($daysBack)	{
			switch($mode)	{
				case 'active':
					...
					$pObj->content.=$pObj->doc->section(sprintf($GLOBALS['LANG']->getLL('lastlogin_main_activeUsersLoggedIn','1'),$daysBack),$content,0,1);
				break;
				default:
					$content = $this->removeOld($id,$pObj,$daysBack);
					$pObj->content.=$pObj->doc->section(sprintf($GLOBALS['LANG']->getLL('lastlogin_removeold_usersWithLastLogin','1'),$daysBack),$content,0,1);
				break;
			}
		}
	}
	...
	function removeOld($id,&$pObj,$daysBack)	{
		...
		if ($action == 'delete') {
			$content .= '<span style="color:red;">'.sprintf($GLOBALS['LANG']->getLL('lastlogin_removeold_deletedSUsers','1'),$num_rows).'</span><br>';
			if ($emailMsg)	{
				$content.='<span style="color:red;">'.sprintf($GLOBALS['LANG']->getLL('lastlogin_removeold_sentDeletedEmails','1'),$num_rows).'</span><br>';
			}
			...
			if ($action=='email')	{
				$content.='<span style="color:red;">'.sprintf($GLOBALS['LANG']->getLL('lastlogin_removeold_sentSWarningEmails','1'),$num_rows).'</span><br>';
			}

			$content.='
			<strong>'.$GLOBALS['LANG']->getLL('lastlogin_removeold_numberOfInactiveUsers','1').'</strong> '.$num_rows.'<br>
			('.sprintf($GLOBALS['LANG']->getLL('lastlogin_removeold_didNotLoginDuring','1'),'<strong>'.$daysBack.'</strong>').')<br>
			<table border="0" cellpadding="1" cellspacing="2">'.implode('
			',$tRows).'</table>';

			$msg = $emailMsg?$emailMsg:sprintf($GLOBALS['LANG']->getLL('lastlogin_removeold_hiNameYouAre','1'),GeneralUtility::getIndpEnv('TYPO3_SITE_URL'),$daysBack,GeneralUtility::getIndpEnv('TYPO3_SITE_URL'));

			$delMsg = sprintf($GLOBALS['LANG']->getLL('lastlogin_removeold_hiNameYouWere','1'),GeneralUtility::getIndpEnv('TYPO3_SITE_URL'));

			$content.='
<br>
<strong>'.$GLOBALS['LANG']->getLL('lastlogin_removeold_sendingAWarningEmail','1').'</strong><br>
<br>
<textarea name="email_msg" rows="20" '.$GLOBALS['TBE_TEMPLATE']->formWidthText(48,'').'>'.GeneralUtility::formatForTextarea($msg).'</textarea><br>
...
'.$GLOBALS['LANG']->getLL('lastlogin_removeold_sendATestTo','1').' <input type="text" name="test_username" value="'.htmlspecialchars(GeneralUtility::_GP('test_username')).'"><br>
<input type="submit" name="sendWarningEmail" value="'.$GLOBALS['LANG']->getLL('lastlogin_removeold_sendWarningEmail','1').'"> - <input type="submit" name="_" value="'.$GLOBALS['LANG']->getLL('lastlogin_removeold_sendUpdate','1').'"><br>
...
<input type="checkbox" name="_DELETE" value="" onclick="
	document.forms[0].sendWarningEmail.value=this.checked?\''.$GLOBALS['LANG']->getLL('lastlogin_removeold_deleteUsers','1').'\':\''.$GLOBALS['LANG']->getLL('lastlogin_removeold_sendWarningEmail','1').'\';
	if (this.checked)	{
...
'.$GLOBALS['LANG']->getLL('lastlogin_showactive_ifYouCheckThis').'
	...
	function showActive($id,&$pObj,$daysBack)	{
		...
		$query = 'SELECT uid,username,email,name,lastlogin FROM fe_users WHERE pid='.intval($id).
			...
			BackendUtility::deleteClause('fe_users').
			' ORDER BY '.(GeneralUtility::inList('username,name,email,lastlogin',$orderBy)?$orderBy.($orderBy=='lastlogin'?' DESC':''):'name');
		$res = $GLOBALS['TYPO3_DB']->sql_query($query);
		$tRows = array();

		$tRows[]='<tr bgcolor="'.$GLOBALS['TBE_TEMPLATE']->bgColor5.'">
			<td nowrap><strong><a href="' . BackendUtility::getModuleUrl($pObj->MCONF['name'], array('id' => $id, 'daysBack' => $daysBack, 'orderby' => 'username')) .'">'.$GLOBALS['LANG']->getLL('lastlogin_showactive_username','1').'</a></strong></td>
			<td nowrap><strong><a href="' . BackendUtility::getModuleUrl($pObj->MCONF['name'], array('id' => $id, 'daysBack' => $daysBack, 'orderby' => 'name')) .'">'.$GLOBALS['LANG']->getLL('lastlogin_showactive_name','1').'</a></strong></td>
			<td nowrap><strong><a href="' . BackendUtility::getModuleUrl($pObj->MCONF['name'], array('id' => $id, 'daysBack' => $daysBack, 'orderby' => 'email')) .'">'.$GLOBALS['LANG']->getLL('lastlogin_showactive_email','1').'</a></strong></td>
			<td nowrap><strong><a href="' . BackendUtility::getModuleUrl($pObj->MCONF['name'], array('id' => $id, 'daysBack' => $daysBack, 'orderby' => 'lastlogin')) .'">'.$GLOBALS['LANG']->getLL('lastlogin_showactive_lastLogin','1').'</a></strong></td>
		</tr>';
		...
		$content .= '
		<strong>'.$GLOBALS['LANG']->getLL('lastlogin_showactive_numberOfActiveUsers','1').'</strong> '.$GLOBALS['TYPO3_DB']->sql_num_rows($res).'<br>
'.sprintf($GLOBALS['LANG']->getLL('lastlogin_sendwarnin_usersAreShownHere'), $daysBack, $daysBack, $this->daySpanBetweenCrAndLogin).'<br>
